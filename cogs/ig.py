...
class Instagram(DiscPy.Cog):
	def __init__(self, bot: DiscPy, db: Database):
		...
		async def on_message(ctx: DiscPy, event: Message):
			...
			if db['server'].find_one(server_id = event.guild_id)['instagram_embed'] == 1:
				url = re.findall(r"(\|{0,2}<?)?((?:(?:(?:https?):(?://)+)(?:www\.)?)instagram\.com/p/.+[^|>])(\|{0,2}>?)?", event.content)
				# [(|| or < or '', url, || or > or '')]
				if url and not ((url[0][0] == '<' and url[0][2] == '>') or (url[0][0] == '||' and url[0][2] == '||')):
					image = Instagram.return_link(url[0][1], msg=event)
					if image:
						embed = Embed(color=0xffcc00, description=f'[Jump directly to instagram]({url[0][1]})\n{event.content.replace(url[0][1], "")}')
		...
		def validate_embed(embeds: List[Embed]):
			if embeds:
				return not (embeds[0].description is None) and '[Jump directly to instagram](https://www.instagram.com/' in embeds[0].description
		...
		async def instagram(ctx: DiscPy, event: Message):
			if db['server'].find_one(server_id = event.guild_id) is None:
				return

			prev = db['server'].find_one(server_id = event.guild_id)['instagram_embed']
			new_val = 0 if prev == 1 else 1
			db['server'].update(dict(server_id = str(event.guild_id), instagram_embed=new_val), ['server_id'])

			await ctx.send_message(event.channel_id, f"instagram embeds: {'on' if new_val == 1 else 'off'}")
